...
.. admonition:: Template Description
     ...
     50TB of memory, 40 GPUs, and 1PB of disk storage jointly funded by faculty
     research grant awards and the University. The cluster supports faculty from over
     20 departments, including: Biomedicine, Business, Chemistry, Computer Science,
     Engineering, Psychology, and Physics. All grant contributions are applied as a service
...
Total resources available on the DEAC Cluster include 101 dedicated nodes, containing 5,712 CPU Cores, 50TB of memory, 1PB storage, 40 GPUs. The facility is a heterogeneous collection of Compute nodes with AMD EPYC based Milan and Genoa 64-bit CPUs connected via 25GB Ethernet, as well as older Compute Nodes with Intel Xeon based Cascade Lake 64-bit CPUs contained within a 10GB ethernet connected Cisco Unified Computing System (UCS). 9 GPU nodes contain all CUDA cores from a mix of 40 GPUs Cards, including V100, A100 (40GB & 80GB), L40S, and H200 models. The cluster storage environment consists primarily of two VAST DataStore storage arraays, hosting up to 1.6PB TB of shared storage via NFS; and an unlimited AWS cloud-based archive. Each storage path is available via 100-gigabit Ethernet from redundant connections; volume configuration follows best practices to provide the utmost reliability, availability, and security to users. Additionally, every node contains at least 2TB of local, temporary scratch space to assist with higher IO throughput.
